...
async def help_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Если вы обнаружили баг то сообщите нам в бота: " @Eternal_paradise_supbot,
        parse_mode="Markdown"
    )
...
async def share_link(update: Update, context: ContextTypes.DEFAULT_TYPE, key: str):
    """Отправляет пользователю ссылку для отправки другим"""
...
def main():
    ...
    logger.info("Бот запущен (с кнопкой «Поделиться ссылкой»)")
    app.run_polling()
