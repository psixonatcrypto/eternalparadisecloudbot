...
async def clear_all_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """СКРЫТАЯ КОМАНДА ДЛЯ АДМИНА: удаляет все файлы из канала и базы данных"""
    if not update.message:
        return
    ...
    await update.message.reply_text("🔄 Начинаю очистку... Это может занять некоторое время.")
    
    # Получаем все ключи из базы
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT key, message_id FROM files')
    rows = c.fetchall()
    conn.close()
    
    if not rows:
        await update.message.reply_text("✅ База данных уже пуста. Файлов для удаления нет.")
        return
    
    deleted_from_channel = 0
    failed_from_channel = 0
    
    # Удаляем каждое сообщение из канала
    for key, message_id in rows:
        try:
            await context.bot.delete_message(chat_id=CHANNEL_ID, message_id=message_id)
            deleted_from_channel += 1
        except Exception as e:
            logger.error(f"Не удалось удалить сообщение {message_id} из канала: {e}")
            failed_from_channel += 1
        await asyncio.sleep(0.3)  # Небольшая задержка, чтобы не спамить API
    ...
    await update.message.reply_text(
        f"✅ Очистка завершена!\n"
        f"🗑 Удалено из канала: {deleted_from_channel}\n"
        f"⚠ Не удалось удалить: {failed_from_channel}\n"
...
def main():
    ...
    app.add_handler(CommandHandler("clear_all", clear_all_files))   # ← СКРЫТАЯ КОМАНДА ДЛЯ АДМИНА
