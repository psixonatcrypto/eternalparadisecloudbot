...
def get_file_info(key):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT file_id, filename, message_id FROM files WHERE key = ?', (key,))
    row = c.fetchone()
    conn.close()
    if row:
        return {"file_id": row[0], "filename": row[1], "message_id": row[2]}
    return None
...
async def delete_file_completely(update: Update, context: ContextTypes.DEFAULT_TYPE, key: str):
    """Удаляет запись из БД и сообщение из канала"""
    info = get_file_info(key)
    if not info:
        await update.callback_query.answer("❌ Файл не найден", show_alert=True)
        return
    ...
    try:
        ...
        await update.callback_query.answer("⚠️ Файл в канале не найден, но ссылка будет удалена", show_alert=True)
...
def main():
    ...
    logger.info("Бот с удалением файлов из канала запущен")
    app.run_polling()
