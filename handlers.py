...
async def my_files(update: Update, context: ContextTypes.DEFAULT_TYPE, parent_id=0, files_page=0):
    try:
        ...
        sort_by = context.user_data.get('sort_by', 'date')
        sort_order = context.user_data.get('sort_order', 'DESC')
...
async def send_file_by_info(chat_id, info, key, bot):
    ...
    if info["media_type"] == "photo":
        ...
        await bot.send_document(chat_id=chat_id, document=info["file_id"], filename=info["filename"], caption=f"💾 {size_str}\n👁 Скачиваний: {info['downloads_count']}")
...
async def save_file_with_options(update: Update, context: ContextTypes.DEFAULT_TYPE, period, upload_id=None):
    try:
        ...
        file_size = temp.get('file_size', 0)
...
async def _save_file(message, context, temp, password=None):
    ...
    try:
        ...
        save_file_info(key, file_id, filename, CHANNEL_ID, sent.message_id, media_type, 
                       user_id, folder_id, password_hash=password_hash, expires_at=expires_at, 
                       file_size=file_size)
    ...
    try:
        ...
        if data == "upload":
            ...
            try:
                ...
                sort_by = parts[4] if len(parts) > 4 else context.user_data.get('sort_by', 'date')
                sort_order = parts[5] if len(parts) > 5 else context.user_data.get('sort_order', 'DESC')
                user_id = update.effective_user.id
                keyboard = folder_keyboard(user_id, parent_id, files_page, sort_by, sort_order)
                await query.message.edit_reply_markup(reply_markup=keyboard)
                await query.answer()
            ...
            try:
                ...
                await query.answer("Ошибка", show_alert=True)
        elif data.startswith("sort_date_"):
            parts = data.split("_")
            parent_id = int(parts[2])
            files_page = int(parts[3])
            context.user_data['sort_by'] = 'date'
            context.user_data['sort_order'] = 'DESC'
            await my_files(update, context, parent_id, files_page)
        elif data.startswith("sort_name_"):
            parts = data.split("_")
            parent_id = int(parts[2])
            files_page = int(parts[3])
            context.user_data['sort_by'] = 'name'
            context.user_data['sort_order'] = 'ASC'
            await my_files(update, context, parent_id, files_page)
        elif data.startswith("sort_size_"):
            parts = data.split("_")
            parent_id = int(parts[2])
            files_page = int(parts[3])
            context.user_data['sort_by'] = 'size'
            context.user_data['sort_order'] = 'DESC'
            await my_files(update, context, parent_id, files_page)
        elif data.startswith("sort_downloads_"):
            parts = data.split("_")
            parent_id = int(parts[2])
            files_page = int(parts[3])
            context.user_data['sort_by'] = 'downloads'
            context.user_data['sort_order'] = 'DESC'
            await my_files(update, context, parent_id, files_page)
            ...
            files_page = int(parts[3])
            sort_by = parts[4]
            current_order = context.user_data.get('sort_order', 'DESC')
            new_order = 'ASC' if current_order == 'DESC' else 'DESC'
            context.user_data['sort_order'] = new_order
